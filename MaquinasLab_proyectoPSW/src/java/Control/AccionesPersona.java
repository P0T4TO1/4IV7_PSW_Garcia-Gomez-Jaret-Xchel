...
public class AccionesPersona {
    ...
    public static int registrarAlumno(MPersona p){
        ...
        try{
            Connection con = Conexion.getConection();
            String q = "insert into persona(alumno_boleta, persona_nombre, persona_appat, "
                    + "persona_apmat, persona_sexo, persona_fechanacimiento, "
                    + "persona_user, persona_pass, grupo_id, rol_id, privilegio_id)"
                    + " values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    public static MPersona buscarPersonaporID(int id){
        ...
        try{
            ...
            String q = "select persona_id, alumno_boleta, empleado_num, persona_nombre, persona_appat, persona_apmat, "
                    + "persona_sexo, persona_fechanacimiento, persona_user, persona_pass, "
                    + "gru.grupo_tipo, ro.rol_tipo, pri.privilegio_tipo from persona as per "
    ...
    public static int actualizarEmpleado(MPersona p){
        ...
        try{
            ...
            String q = "update persona set empleado_num = ?, persona_nombre = ?, persona_appat = ?," 
                      +  "persona_apmat = ?, persona_sexo = ?, persona_fechanacimiento = ?, persona_user = ?," 
                      +  "persona_pass = ? " 
                      +  "where persona_id = ?";
    ...
    public static int actualizarAlumno(MPersona p){
        ...
        try{
            ...
            String q = "update persona set alumno_boleta = ?, persona_nombre = ?, persona_appat = ?," 
                      +  "persona_apmat = ?, persona_sexo = ?, persona_fechanacimiento = ?, persona_user = ?," 
                      +  "persona_pass = ? " 
                      +  "where persona_id = ?";
    ...
    public static int actulizarPersona(MPersona p){
        int estatus = 0;
        
        try{
            Connection con = Conexion.getConection();
            
            String q = "update persona set alumno_boleta = ?, empleado_num = ?, persona_nombre = ?, persona_appat = ?," 
                      +  "persona_apmat = ?, persona_sexo = ?, persona_fechanacimiento = ?, persona_user = ?," 
                      +  "persona_pass = ?, grupo_id = ?, rol_id = ?, privilegio_id = ?" 
                      +  "where persona_id = ?";
            
            PreparedStatement ps = con.prepareStatement(q);

            ps.setInt(1, p.getAlumno_boleta());
            ps.setInt(2, p.getEmpleado_num());
            ps.setString(3, p.getPersona_nombre());
            ps.setString(4, p.getPersona_appat());
            ps.setString(5, p.getPersona_apmat());
            ps.setString(6, p.getPersona_sexo());
            ps.setString(7, p.getPersona_fechaNacimiento());
            ps.setString(8, p.getPersona_user());
            ps.setString(9, p.getPersona_password());
            ps.setInt(10, p.getGrupo_id());
            ps.setInt(11, p.getRol_id());
            ps.setInt(12, p.getPrivilegio_id());
            ps.setInt(13, p.getPersona_id());

            estatus = ps.executeUpdate();

            System.out.println("Se actualizo el admin");
            con.close();
